...
DEFAULT_DEVICE_ADDRESS = (
    0x27  # 7 bit address (will be left shifted to add the read write bit)
)
DESIRED_DEVICE_ADDRESS = (
    0x28  # 7 bit address (will be left shifted to add the read write bit)
)
...
TR_RE7 = "(OFF)"
TR_ON = re.compile(
    TR_RE1 + TR_RE2 + TR_RE3 + TR_RE4 + TR_RE5 + TR_RE6, re.IGNORECASE | re.DOTALL
)
TR_OFF = re.compile(
    TR_RE1 + TR_RE2 + TR_RE3 + TR_RE4 + TR_RE5 + TR_RE7, re.IGNORECASE | re.DOTALL
)
...
SQ_RE7 = "(CLOSED)"
SQ_ON = re.compile(
    SQ_RE1 + SQ_RE2 + SQ_RE3 + SQ_RE4 + SQ_RE5 + SQ_RE6, re.IGNORECASE | re.DOTALL
)
SQ_OFF = re.compile(
    SQ_RE1 + SQ_RE2 + SQ_RE3 + SQ_RE4 + SQ_RE5 + SQ_RE7, re.IGNORECASE | re.DOTALL
)
...
def lcd_draw_filled_rectangle(
    ADDRESS, X1, Y1, X2, Y2, ForeGroundColor, BackGroundColor
):
...
def lcd_set_foreground(ADDRESS, color):
    ...
    return socket.inet_ntoa(
        fcntl.ioctl(
            s.fileno(), 0x8915, struct.pack("256s", network[:15])  # SIOCGIFADDR
        )[20:24]
    )
...
lcd_write_line(DESIRED_DEVICE_ADDRESS, "       TNARC VE6HM")
